...
/** Paginated list of object keys under a prefix (e.g. `listing/`). Pass `""` for the whole bucket. Omits zero-byte “folder” markers. */
...
/** Every object key in the configured bucket (paginated). */
export async function listAllR2ObjectKeys(): Promise<string[]> {
  return listR2ObjectKeysWithPrefix("");
}

/**
 * Delete objects by exact key (no path guard). Intended only for keys verified as orphans against
 * {@link listAllR2ObjectKeys} and DB references.
 */
export async function deleteR2ObjectsByKeysForPrune(keys: readonly string[]): Promise<number> {
  ...
  let n = 0;
  for (const Key of keys) {
    if (!Key || Key.includes("\0") || Key.endsWith("/")) continue;
    try {
      await client.send(new DeleteObjectCommand({ Bucket: bucket, Key }));
      n += 1;
    } catch {
...
/** Pending owner supplement awaiting admin promotion to {@link shopListingSupplementImageObjectKey}. */
export function shopListingSupplementPendingImageObjectKey(shopId: string, listingId: string): string {
  return `shops/${shopId}/listing-supplement-pending/${listingId}.webp`;
}
...
/**
 * Same rules as {@link listingSupplementImageUrlToObjectKey} for the pending supplement key.
 */
export function listingSupplementPendingImageUrlToObjectKey(
  publicUrl: string,
  shopId: string,
  listingId: string,
): string | null {
  const expectedKey = shopListingSupplementPendingImageObjectKey(shopId, listingId);
  const strict = publicUrlToR2ObjectKey(publicUrl.trim());
  if (strict === expectedKey) return strict;
...
export function sanitizeShopListingOwnerSupplementPendingImageUrlForDisplay(
  publicUrl: string | null | undefined,
  shopId: string,
  listingId: string,
): string | null {
  if (!publicUrl?.trim()) return null;
  return listingSupplementPendingImageUrlToObjectKey(publicUrl, shopId, listingId)
    ? publicUrl.trim()
    : null;
}
...
/**
 * Deletes the listing supplement object from Cloudflare R2 (`DeleteObject` on {@link shopListingSupplementImageObjectKey}).
 * @returns `true` if nothing to do (R2 not configured), or delete succeeded. `false` if R2 is configured but delete failed after a short retry.
 */
export async function deleteShopListingSupplementObject(shopId: string, listingId: string): Promise<boolean> {
...
/**
 * Deletes the pending listing supplement object (`listing-supplement-pending/{id}.webp`).
 * @returns Same contract as {@link deleteShopListingSupplementObject}.
 */
export async function deleteShopListingSupplementPendingObject(
  shopId: string,
  listingId: string,
): Promise<boolean> {
  if (!isR2UploadConfigured()) return true;
  ...
  const Key = shopListingSupplementPendingImageObjectKey(shopId, listingId);
  const client = r2S3Client();
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      await client.send(new DeleteObjectCommand({ Bucket: bucket, Key }));
...
/**
 * Copies pending WebP bytes to the canonical live supplement key, deletes the pending object,
 * and returns the public URL for the live key.
 *
 * Explicitly removes any existing live object first so the prior storefront image is cleared from R2
 * before the new object is written (then pending is deleted).
 */
export async function promoteShopListingSupplementPendingToLive(params: {
  shopId: string;
  listingId: string;
  pendingPublicUrl: string;
}): Promise<{ ok: true; livePublicUrl: string } | { ok: false; error: string }> {
  const { shopId, listingId, pendingPublicUrl } = params;
  const pendingKey = listingSupplementPendingImageUrlToObjectKey(
    pendingPublicUrl,
    shopId,
    listingId,
  );
  if (!pendingKey) {
    return { ok: false, error: "pending_url_invalid" };
  }
  ...
  const bucket = readR2BucketName();
  if (!bucket) {
    return { ok: false, error: "r2_bucket_missing" };
  }

  const liveKey = shopListingSupplementImageObjectKey(shopId, listingId);
  const priorLiveRemoved = await deleteShopListingSupplementObject(shopId, listingId);
  if (!priorLiveRemoved) {
    console.warn("[r2] promote supplement: prior live object delete incomplete", { shopId, listingId });
  }
  ...
  if (!copied) {
    try {
      ...
      return { ok: false, error: "promote_copy_failed" };
    }
  }

  const delOk = await deleteShopListingSupplementPendingObject(shopId, listingId);
  if (!delOk) {
    return { ok: false, error: "promote_delete_pending_failed" };
  }

  let livePublicUrl: string;
  try {
    livePublicUrl = publicHttpsUrlForR2ObjectKey(liveKey);
  } catch {
    return { ok: false, error: "r2_public_base_missing" };
  }

  if (!listingSupplementImageUrlToObjectKey(livePublicUrl, shopId, listingId)) {
    return { ok: false, error: "live_url_validation_failed" };
  }
