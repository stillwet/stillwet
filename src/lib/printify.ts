...
export const PRINTIFY_ADMIN_FETCH_TIMEOUT_MS = 15_000;
...
async function printifyAuthorizedFetch(
  ...
  const signal =
    init?.signal ?? AbortSignal.timeout(options?.timeoutMs ?? PRINTIFY_FETCH_TIMEOUT_MS);
  return fetch(url, {
    ...init,
    headers,
    signal,
...
export async function fetchPrintifyShops(options?: {
  timeoutMs?: number;
}): Promise<PrintifyShopSummary[]> {
  const res = await printifyAuthorizedFetch("/shops.json", undefined, options);
