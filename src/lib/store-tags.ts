...
import { getPlatformStoreTagsFromSnapshot } from "@/lib/platform-store-tags-snapshot";
import {
...
const STORE_TAGS_REVALIDATE_SECONDS = 30 * 60;
...
const getStoreTagsFromDb = unstable_cache(
  async () => {
    try {
      const fromSnapshot = await getPlatformStoreTagsFromSnapshot();
