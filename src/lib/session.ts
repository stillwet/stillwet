...
export type ShopOwnerSession = {
  shopUserId?: string;
  /** When set, a recent password login is waiting on email device confirmation. */
  pendingTwoFactorShopUserId?: string;
};
...
function cartSessionPassword(): string {
  ...
  if (!loggedCartSessionSecretFallback) {
    ...
    console.error(
      "[stillwet] SESSION_SECRET is missing or shorter than 32 characters. Store pages use a fallback cart key so the shop can load; set SESSION_SECRET in Vercel (see .env.example) and redeploy.",
    );
  }
  return "stillwet-fallback-cart-session-key!!";
}

const cartBase: Omit<SessionOptions, "password"> = {
  cookieName: "stillwet_cart",
  cookieOptions: {
...
const shopOwnerBase: Omit<SessionOptions, "password"> = {
  cookieName: "stillwet_shop_owner",
  cookieOptions: {
    secure: process.env.NODE_ENV === "production",
    httpOnly: true,
    sameSite: "lax",
...
/** Next.js throws when `cookies()` runs during a static prerender — expected, not a real failure. */
function isNextDynamicServerUsageError(e: unknown): boolean {
...
export async function getCartSessionReadonly(): Promise<CartSession> {
  try {
    ...
    if (!isNextDynamicServerUsageError(e)) {
...
/**
 * Lightweight cart read for header badges only. It intentionally avoids DB hydration/validation;
 * cart and checkout pages still call `getCartSessionReadonly()` for authoritative reads.
 */
export async function getCartSessionBadgeReadonly(): Promise<Pick<CartSession, "items">> {
  try {
    const session = await getIronSession<CartSession>(await cookies(), {
      ...cartBase,
      password: cartSessionPassword(),
    ...
    if (!isNextDynamicServerUsageError(e)) {
      console.error("[getCartSessionBadgeReadonly]", e);
    }
    return { items: {} };
  }
}
 ...
 * corrupt `stillwet_admin` cookie yields `{ isAdmin: false }` so `/admin` can redirect instead of 500.
 */
