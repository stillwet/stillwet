...
/**
 * Recursive `.next` size scans can take minutes on large trees (local dev, production hosts with big
 * caches). Never run them during a normal admin request unless explicitly enabled.
 */
function adminMeasureNextBuildBytesEnabled(): boolean {
  return process.env.ADMIN_MEASURE_NEXT_BUILD_BYTES?.trim() === "true";
}
 ...
 *
 * Byte fields are null unless `ADMIN_MEASURE_NEXT_BUILD_BYTES=true` — measuring avoids blocking `/admin`
 * on unbounded filesystem walks (see history in repo).
 */
export async function getAdminDeployFootprint(): Promise<AdminDeployFootprint> {
  ...
  const measure = adminMeasureNextBuildBytesEnabled();

  const nextBuildArtifactBytes =
    nextBuildDirPresent && measure
      ? await getNextArtifactSizeBytesExcludingDevCaches(nextDir)
      : null;

  const nextBuildBytes =
    nextBuildDirPresent && measure ? await getDirectoryTotalSizeBytes(nextDir) : null;
