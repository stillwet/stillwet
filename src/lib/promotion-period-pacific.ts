...
/** Pacific civil calendar date `YYYY-MM-DD` for dashboard sales snapshot period keys. */
export function pacificCalendarDateKey(instant: Date = new Date()): string {
...
export function pacificMidnightUtc(y: number, mo: number, d: number): Date {
  ...
  while (hi - lo > 1000) {
    ...
    const p = pacificParts(new Date(mid));
    const ord = p.y * 10_000 + p.m * 100 + p.d;
    const targetOrd = y * 10_000 + mo * 100 + d;
    const secs = p.hour * 3600 + p.minute * 60 + p.second;
    if (ord < targetOrd || (ord === targetOrd && secs < 1)) {
      lo = mid + 1;
    } else {
      hi = mid;
    }
...
export function getPromotionPeriodIndexContaining(instant: Date): number {
  ...
  const anchorMs = promotionPeriodZeroStartUtc().getTime();
  if (t < anchorMs) return 0;
  const approxPeriodMs = PROMOTION_PERIOD_DAYS * 86_400_000;
  const rough = Math.floor((t - anchorMs) / approxPeriodMs);
  let lo = Math.max(0, rough - 2);
  let hi = rough + 3;
  while (lo < hi) {
...
const PLACEMENT_PERIOD_TIER_LABELS = ["Current", "Upcoming", "Following"] as const;

/** Relative tier for placement offsets 0 / 1 / 2 from the active Pacific period. */
export function placementPeriodTierLabel(offset: 0 | 1 | 2): string {
...
export function formatPromotionPlacementPeriodChoiceLabel(
  ...
  return `${placementPeriodTierLabel(offset)} — ${formatPromotionPlacementPeriodMmDdRangeForIndex(periodIndex)}`;
}
