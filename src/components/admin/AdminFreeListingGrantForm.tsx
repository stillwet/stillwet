...
import {
  adminGrantShopFreeListingSlotsForm,
  verifyAdminShopSlugForFreeListingGrant,
...
export function AdminFreeListingGrantForm(props: {
  ...
  const resolveShop = useCallback(
    async (raw: string): Promise<AdminShopSlugPick | null> => {
      ...
      const r = await verifyAdminShopSlugForFreeListingGrant(raw);
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    void (async () => {
      ...
      setIsGranting(true);
      try {
        await adminGrantShopFreeListingSlotsForm(fd);
      } catch (err) {
        console.error("[AdminFreeListingGrantForm] grant failed", err);
  ...
  return (
    <form onSubmit={onSubmit} className="mt-4 space-y-1.5">
      ...
      <button
        type="submit"
        disabled={isGranting || !normalizedInput}
      ...
      >
        {isGranting ? "Granting…" : "Grant slots"}
      </button>
      </div>
      {selectedShop ? (
