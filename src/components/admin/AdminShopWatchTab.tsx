  ...
  return (
    <div className="mt-4 first:mt-0">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex w-full items-center gap-2 rounded-lg border border-zinc-800/70 bg-zinc-950/40 px-3 py-2 text-left transition hover:border-zinc-700 hover:bg-zinc-900/40"
      >
        <svg
        ...
        <span className={`flex min-w-0 flex-1 flex-wrap items-baseline gap-x-2 text-[11px] font-medium uppercase tracking-wide ${titleClass}`}>
          <span>{title}</span>
