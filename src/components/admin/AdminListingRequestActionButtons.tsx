...
import {
  ...
  adminRejectListingSupplementImage,
} from "@/actions/admin-marketplace";
import {
  LISTING_REJECT_REASON_VALUES,
  LISTING_SUPPLEMENT_REJECT_REASON_VALUES,
  type ListingRejectReasonValue,
...
export function AdminListingRejectReasonRadios({
  legendText = "Rejection reason",
  disabled = false,
  legendId = rejectReasonFieldId,
  reasonValues = LISTING_REJECT_REASON_VALUES,
}: {
  /** Omit visible legend when empty (custom image queue); fieldset keeps `aria-label`. */
  legendText?: string;
  disabled?: boolean;
  /** Unique when multiple reject forms appear on one page (e.g. custom image queue). */
  legendId?: string;
  /** Subset of listing reasons (e.g. custom image reject omits artwork). */
  reasonValues?: readonly ListingRejectReasonValue[];
}) {
  const showLegend = legendText.trim().length > 0;
  return (
    <fieldset
      disabled={disabled}
      aria-label={showLegend ? undefined : "Rejection reason"}
      className="min-w-0 space-y-1.5 border-0 p-0 transition-opacity duration-200"
    >
      {showLegend ? (
        <legend id={legendId} className="text-[11px] font-medium text-zinc-500">
          {legendText}
        </legend>
      ) : null}
      <div
        className={showLegend ? "mt-1 space-y-1.5" : "space-y-1.5"}
        role="group"
        aria-labelledby={showLegend ? legendId : undefined}
      >
...
export function AdminListingSupplementImageRejectForm({
  listingId,
  legendId,
}: {
  listingId: string;
  ...
  return (
    <form
      action={adminRejectListingSupplementImage}
    ...
    >
      ...
      <AdminListingRejectReasonRadios
        legendText=""
        legendId={legendId}
        reasonValues={LISTING_SUPPLEMENT_REJECT_REASON_VALUES}
      />
    </form>
  );
}
...
function AdminRejectSubmitButton({
  ...
  label?: string;
  pendingLabel?: string;
  /** Merged after base reject styles (e.g. match supplement Approve height). */
  buttonClassName?: string;
}) {
  const { pending } = useFormStatus();
  const disabled = pending || formDisabled;
  const base = pending ? rejectPending : rejectIdle;
  const className = buttonClassName ? `${base} ${buttonClassName}` : base;
  return (
    <button type="submit" disabled={disabled} className={className} aria-busy={pending}>
      {pending ? pendingLabel : label}
