...
import {
  ...
  promoteShopListingSupplementPendingToLive,
  putPublicR2Object,
...
import {
  ...
  parseListingSupplementRejectReason,
...
export async function adminFreezeShopListing(formData: FormData) {
  ...
  await prisma.shopListing.update({
    ...
    data: {
      active: false,
      adminRemovedFromShopAt: new Date(),
...
function listingSupplementApprovalLabel(row: {
  requestItemName: string | null;
  product: { name: string };
}): string {
  const shop = row.requestItemName?.trim() || row.product.name;
  const catalog = row.product.name.trim() || shop;
  return `"${shop}" (${catalog})`;
}

/** Promotes pending owner supplement to live storefront image; notifies the shop. */
export async function adminApproveListingSupplementImage(formData: FormData): Promise<void> {
  await requireAdmin();
  const listingId = String(formData.get("listingId") ?? "").trim();
  if (!listingId) return;
  ...
  if (listing.creatorRemovedFromShopAt != null || listing.adminRemovedFromShopAt != null) return;
  const pendingRaw = listing.ownerSupplementPendingImageUrl?.trim() ?? "";
  if (!pendingRaw) return;
  if (!listingSupplementPendingImageUrlToObjectKey(pendingRaw, listing.shopId, listingId)) return;

  const promoted = await promoteShopListingSupplementPendingToLive({
    shopId: listing.shopId,
    listingId,
    pendingPublicUrl: pendingRaw,
  });
  if (!promoted.ok) {
    console.error("[adminApproveListingSupplementImage]", listingId, promoted.error);
    return;
  }

  const label = listingSupplementApprovalLabel(listing);
  await prisma.shopListing.update({
    where: { id: listingId },
    data: {
      ownerSupplementImageUrl: promoted.livePublicUrl,
      ownerSupplementPendingImageUrl: null,
      ownerSupplementPendingSubmittedAt: null,
    },
  });
  await prisma.shopOwnerNotice.create({
    data: {
      shopId: listing.shopId,
      kind: "listing_supplement_image_approved",
      relatedListingId: listingId,
      body: `Your custom storefront image for ${label} was approved — it's live on your listing now.`,
    },
  });
  ...
  revalidatePath("/dashboard");
  revalidatePath(`/s/${listing.shop.slug}`);
  revalidatePath(`/s/${listing.shop.slug}/product/${listing.product.slug}`);
}

/** Deletes pending supplement from R2 and clears pending fields; live image is unchanged; notifies the shop. */
export async function adminRejectListingSupplementImage(formData: FormData): Promise<void> {
  await requireAdmin();
  const listingId = String(formData.get("listingId") ?? "").trim();
  const rejectReason = parseListingSupplementRejectReason(formData.get("rejectReason"));
  if (!listingId || !rejectReason) return;
  ...
  const removed = await deleteShopListingSupplementPendingObject(listing.shopId, listingId);
  if (!removed) {
    console.error("[adminRejectListingSupplementImage] R2 delete failed", listingId);
    return;
  }
  ...
  const regulationsUrl = `${base}/shop-regulations`;
  const noticeBody =
    rejectReason === "regulations"
      ? `Your custom image request for ${label} was rejected for not following [item regulations](${regulationsUrl})`
      : `Your custom image request for ${label} was rejected.`;
  await prisma.shopListing.update({
    where: { id: listingId },
    data: {
      ownerSupplementPendingImageUrl: null,
  ...
  await prisma.shopOwnerNotice.create({
    data: {
      shopId: listing.shopId,
      kind: "listing_supplement_image_rejected",
      relatedListingId: listingId,
      body: noticeBody,
    },
  });
