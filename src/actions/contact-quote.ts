...
export async function submitMerchQuoteContact(
  ...
  const subject = replaceContactQuotePlaceholders(MERCH_QUOTE_DEFAULT_SUBJECT_TEMPLATE, {
    name,
    email,
    message,
  });
  const text = replaceContactQuotePlaceholders(MERCH_QUOTE_DEFAULT_TEXT_TEMPLATE, {
    name,
    email,
    message,
  });
