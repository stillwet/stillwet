...
import type { AdminSavePopularItemsFeaturedState } from "@/actions/admin-popular-items-featured-state";
...
export async function adminSavePopularItemsFeaturedProductIdsForm(
  _prev: AdminSavePopularItemsFeaturedState,
  formData: FormData,
): Promise<AdminSavePopularItemsFeaturedState> {
  await requireAdmin();
  ...
  if (normalized.length > HOME_HOT_CAROUSEL_MAX_ITEMS) {
    return { ok: false, error: `At most ${HOME_HOT_CAROUSEL_MAX_ITEMS} products.` };
  }
  ...
  try {
    await prisma.shop.update({
      ...
      data: { popularItemsFeaturedProductIds: normalized },
    });
  } catch (e) {
    console.error("[adminSavePopularItemsFeaturedProductIdsForm]", e);
